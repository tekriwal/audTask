function [ expData ] = extractWaveforms_Clz_v01b(spikeData, spkThresh, noisThresh, minDist, Fs)
...
if sum(spikeData > noisThresh) == 0
    ...
    [pNoiseLocs ,  ~] = peakseek(spikeData , 1 , spkThresh);
...
%     [~, nNoiseLocs] = findpeaks(abs(ridP),'MinPeakHeight',noisThresh);
    [nNoiseLocs ,  ~] = peakseek(spikeData , 1 , spkThresh);
end

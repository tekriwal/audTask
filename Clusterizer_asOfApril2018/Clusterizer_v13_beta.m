function varargout = Clusterizer_v13_beta(varargin)
% CLUSTERIZER_V13_BETA MATLAB code for Clusterizer_v13_beta.fig
%      CLUSTERIZER_V13_BETA, by itself, creates a new CLUSTERIZER_V13_BETA or raises the existing
%      singleton*.
%
%      H = CLUSTERIZER_V13_BETA returns the handle to a new CLUSTERIZER_V13_BETA or the handle to
%      the existing singleton*.
%
%      CLUSTERIZER_V13_BETA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CLUSTERIZER_V13_BETA.M with the given input arguments.
%
%      CLUSTERIZER_V13_BETA('Property','Value',...) creates a new CLUSTERIZER_V13_BETA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Clusterizer_v13_beta_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Clusterizer_v13_beta_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Clusterizer_v13_beta

% Last Modified by GUIDE v2.5 10-Apr-2018 14:55:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Clusterizer_v13_beta_OpeningFcn, ...
    'gui_OutputFcn',  @Clusterizer_v13_beta_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Clusterizer_v13_beta is made visible.
function Clusterizer_v13_beta_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Clusterizer_v13_beta (see VARARGIN)

% Choose default command line output for Clusterizer_v13_beta
...
% UIWAIT makes Clusterizer_v13_beta wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = Clusterizer_v13_beta_OutputFcn(hObject, eventdata, handles)
...
switch handles.LoadSelection
    ...
    case 's'
        ...
        if handles.activeFileNum == height(handles.fedTable)
            ...
            while lastC
                if tmpAFN == height(handles.fedTable) && handles.fedTable.Analyzed(tmpAFN)
...
switch stage
    ...
    case 'new'
        ...
        sampData = handles.spikeDataRaw;
        plotSampData = decimate(double(sampData),3);
        plot(plotSampData,'k');
    ...
    case 'threshold'
        ...
        hold on
        %         plot(handles.waveForms.posWaveInfo.positions(:),...
        %              handles.waveForms.posWaveInfo.waves(:),'r')
        ...
        %
        %             line(handles.waveForms.posWaveInfo.positions,...
        %                 handles.waveForms.posWaveInfo.waves,'Color','r')
        %
        %         else
        %             line(handles.waveForms.negWaveInfo.positions,...
        %                 handles.waveForms.negWaveInfo.waves,'Color','r')
        %         end
        %
        ...
        if handles.pnFlag == 1
            axes(handles.spikeDact)
            line([0 length(plotSampData)] , [voltthresh voltthresh],'Color', 'c', 'LineStyle','--','LineWidth',2);
        elseif handles.pnFlag == 0
            axes(handles.spikeDact)
            line([0 length(plotSampData)] , [voltthresh*-1 voltthresh*-1],'Color', 'c', 'LineStyle','--','LineWidth',2);
        end

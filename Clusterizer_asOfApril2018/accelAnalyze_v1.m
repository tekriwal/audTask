function [emgDatOut] = accelAnalyze_v1(inPUTS)
...
taskbase_io = AOstart_adapter_V3(taskbase_io, Ephys_struct, 'trialStart', rData, index_errors, caseNum, cutNearOEs); %this should add output field in taskbase struct which contains the start of each trial in the time of the alphaomega clock.
...
if sum(contains(tmpEphysFields,'ACCEL_P')) == 0
    disp('NEED TO RUN accelProcess_V2!!!')
    return
end
%AT 3/28/20; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [outSignal , emgT] = remArtifact(inSignal , emg , ttlInfo , nFs , trimOpt)
...
% 
% t2 = t; 
% t2(signal < -10) = [];           % Temporary Time Vector
% signal(signal < -10) = [];                              % Set Signal ‘Dips’ To ‘Empty’
%                                 % Set Corresponding Time Of ‘Dips’ To ‘Empty’
% signal_new = interp1(t2, signal, t, 'spline');          % Interpolate To Fill Missing Data
% figure(1)
% plot(t,signal_new,'-r')
    

% badINDs = find(badIndexes);
% % NEW smooth bad index
% for si = 1:length(badINDs)
%    
%     tmpBI = badINDs(si);
%     tmpRegion = round(linspace(tmpBI-6,tmpBI+6,12));
%     tmpRegionN = tmpRegion(~ismember(tmpRegion,tmpBI));
%     
%     newSignal(tmpBI) = mean(tmpRegionN);
%     
%     
%     
%     

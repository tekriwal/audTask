...
function [L_SG, R_SG, L_IS, R_IS, L_SG_correct_structs, R_SG_correct_structs, L_IS_correct_structs,  R_IS_correct_structs, Errors] = io_taskindexing_AT_V4(rsp_master_v3, input_matrix_rndm, exclusionfilter, index_errorsPtHeard)
...
%AT adding below 6/24/20 to account errors that pt actually heard, note that we're not counting
%first three trials
Errors.ISerrors_errorsPtHeard = sum(index_errorsPtHeard == 1 & ISindex == 1); %gives numb of errors that occurred during IS trials
Errors.SGerrors_errorsPtHeard = sum(index_errorsPtHeard(1:end)) - Errors.ISerrors_errorsPtHeard;
Errors.sumErrorsindex_errorsPtHeard = sum(index_errorsPtHeard(1:end));

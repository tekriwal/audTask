...
if cutNearOEs == 1
    for i = 1:length(rData)
        ...
        if rData(i).Postholdtime < threshRxn && rData(i).rtime > 0
            nearOEcounter = nearOEcounter+1;
        end
    end
end
...
disp(['index error sum for behav file adapter is ', num2str(sum(index_errors))])
disp(['Number of indexxed nearOE is ', num2str(sum(nearOEcounter))])

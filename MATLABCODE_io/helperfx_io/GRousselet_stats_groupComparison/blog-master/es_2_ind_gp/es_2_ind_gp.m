...
for C = 1:Nc
    ...
    %AT 1/17/19 - editted below because fx not found - I think c and d
    %refer to continuous and discrete, so I believe I want to use the
    %gcmi_cc fx anyway
    mi = gcmi_cc(g1,gg2);
...
%AT editted below from gcmi_cd to _cc
fprintf('MI    = %.4f vs. %.4f\n',gcmi_cc(g1,g2),gcmi_cc(g1,mg2))
fprintf('Q     = %.4f vs. %.4f\n',qhat(g1,g2),qhat(g1,mg2))
fprintf('------------------------------------\n')

%% when units matter: all pairwise differences - independent groups

rng(1) % set seed for reproducible  results

Nt = 20; % number of trials in each group
Nt_AT = 20;
g1 = randn(Nt_AT,1); % group 1 = sample from a normal distribution

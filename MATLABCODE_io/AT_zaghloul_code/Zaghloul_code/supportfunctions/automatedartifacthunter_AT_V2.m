%v2 on 1/21/20, goal is to have this code also process the raw-ish LFP
%files 
...
function [events_all_clean, powerMat_all_clean, phaseMat_all_clean, LFP_joint_clean]=automatedartifacthunter_AT_V2(filename, events_all, powerMat_all, phaseMat_all, LFP_joint, time, freqs, Zthresh, freq_indexes, plotdata)
...
events_all=events_all(order); powerMat_all=powerMat_all(order,:,:); phaseMat_all=phaseMat_all(order,:,:); LFP_joint=LFP_joint(order,:);
events_all_clean=events_all; powerMat_all_clean=powerMat_all; phaseMat_all_clean=phaseMat_all;LFP_joint_clean = LFP_joint; % make copies of the original data

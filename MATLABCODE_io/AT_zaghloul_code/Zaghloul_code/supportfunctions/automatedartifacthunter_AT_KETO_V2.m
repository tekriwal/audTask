%AT on 5/1/19, editting this so compatible with rsp output structure




function [events_all_clean, powerMat_all_clean, phaseMat_all_clean]=automatedartifacthunter_AT_KETO_V2(filename, events_all, powerMat_all, phaseMat_all, time, freqs, Zthresh, freq_indexes, plotdata)
...
% [~, order]=sort([events_all.Wholetrial]);  % I sort it for ploting reasons. I want to see adjacent artifacts if they exists.


% events_all=events_all(order);
% powerMat_all=powerMat_all(:);
% phaseMat_all=phaseMat_all(:);
events_all_clean=events_all;
powerMat_all_clean=powerMat_all;
phaseMat_all_clean=phaseMat_all; % make copies of the original data

if length(events_all)~=size(powerMat_all,1);
    keyboard;
end % events all should correspond to the the power given to this function. Make sure it is the case.
...
%AT 11/1/19 - so there's two things going on above. First, bsxfun subtracts
%the mean power from each of the normalized samples. Second, each of these new values is now divded by the STD

freqMat_all=mean(powerMat_allnorm(:,freq_indexes,:));

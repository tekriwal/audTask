...
freqs=2.^((8:54)/8); %AT, I'm not too sure what the fx of this is
...
%         events_joint = [events_SGjoint, events_ISjoint]; %unsure if this
%         will be used, but prob
...
%        ioTask_index_timestamps(ioTask_index_timestamps==0) = [];

%         for t=1:length(events_joint)
%             startindex=ioTask_index_timestamps(t)-Offset*1000-Buffer; 
%             endindex=startindex-1+Duration*1000+2*Buffer;
%             LFP_joint(t,:)=LFP_wholerecordings(startindex:endindex);
%         end
%                 

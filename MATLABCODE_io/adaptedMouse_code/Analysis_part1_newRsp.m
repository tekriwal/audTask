%AT 11/29/19; building new code pipeline off of what I previously had
%written. I've just moved all data to box and added the box.com 'drive' to
%help sync things


%path to filename for excel sheet
filepath = '/Users/andytek/Box/Auditory_task_SNr/IntraoperativeTracking_Data_Sheet.xlsx';

%input function specifics here
Case_number = 4;

%need to set import options to allow for names of rows to be unchanged when
%importing, generally I think this is an important feature of tables. It
%lets us have a lot of control over what form and which columns of data are
%imported 
...
%step 1 is to load in the excel data sheet that will determine the names
%and such of files

% sheet = 1;
% [num,txt,raw] = xlsread(filepath, sheet);

Table_master = readtable(filepath, 'PreserveVariableNames', true, 'ReadRowNames', true); %need to set the preservevariablenames to true if we want it to preserve the same names as what I have used in the sheet itself

%need to convert the date into something matlab can interpret
Table_master.IODate = datetime(Table_master.IODate,'InputFormat','dd-MMM-yy'); %


%PART 1 - ephys analysis; use on the newer versions of the rsp data

%AT 9/10/18 - note, this version of code should be used for all outputs
%after April 2018

%there's a few lines of code to change based on things like whether
%thresholded at positive or negative waveform, what iteration of 'rsp' the
%output is in (we've added more fields to the task output struct as we've gone along)
...
Table_master = Table_master('RowNames', CaseNumber_sucessful_ )
Table_caseNumber = Table_master


%PT6
load('rsp_master_v3_CASE1_45trials.mat')

load('/Users/andytek/Box/Auditory_task_SNr/Data/io_neural_data/05_NeuroPhysData_04042018a/BlwTrgt_6_01397.mat')

load('spkData_b6_01397_1_clzD.mat') %AT filled in 10_30
load('input_matrix_rndm_April4_case1.mat')
rData = rsp_master_v3;
...
% added 3/8/17 to select out specific clusters (ie hardcoded struct indexing); we can ignore this next block of code so long as we've only exported one cluster per spike file!
% spikeDATA_cluster1 = spikeDATA;
% % fnames = fieldnames(spikeDATA);
% X = spikeDATA_cluster1.clustIDS;
% cluster1 = X(:)~=1; %this is now a logical index for only cluster 1 neurons
% spikeDATA_cluster1.clustIDS(cluster1,:) = []; %remove the whole row
% 
% % spikeDATA_cluster1.waveforms.posWaveInfo.waves(:,cluster1) = [];
% % spikeDATA_cluster1.waveforms.posWaveInfo.posLocs(:,cluster1) = [];
% spikeDATA_cluster1.waveforms.negWaveInfo.waves(:,cluster1) = [];
% spikeDATA_cluster1.waveforms.negWaveInfo.neglocs(:,cluster1) = [];
% 
% spikeDATA_cluster1.waveforms.allWaves.waves(:,cluster1) = [];
% spikeDATA_cluster1.waveforms.allWavesInfo.alllocs(:,cluster1) = [];
% spikeDATA_cluster1.features(cluster1,:) = [];
% spikeDATA_cluster1.atIO.timestamps(:,cluster1) = [];
% spikeDATA_cluster1.atIO.clustIDs(cluster1,:) = [];
% % (cluster1,:) = []
% 
% spikeDATA = spikeDATA_cluster1;

% spikeDATA_cluster2 = spikeDATA;
% fnames = fieldnames(spikeDATA);
% X = spikeDATA_cluster2.clustIDS;
% cluster1 = X(:)~=2; %this is now a logical index for only cluster 2 neurons
% spikeDATA_cluster2.clustIDS(cluster1,:) = []; %remove the whole row
% spikeDATA_cluster2.waveforms.posWaveInfo.waves(:,cluster1) = [];
% spikeDATA_cluster2.waveforms.posWaveInfo.posLocs(:,cluster1) = [];
% spikeDATA_cluster2.waveforms.allWaves.waves(:,cluster1) = [];
% spikeDATA_cluster2.waveforms.allWavesInfo.alllocs(:,cluster1) = [];
% spikeDATA_cluster2.features(cluster1,:) = [];
% spikeDATA_cluster2.atIO.timestamps(:,cluster1) = [];
% spikeDATA_cluster2.atIO.clustIDs(cluster1,:) = [];
% % (cluster1,:) = []
% spikeDATA = spikeDATA_cluster2;
...
for i = 1:length(rData)
    ...
    behavioral_matrix(i,11) = input_matrix_rndm(i);

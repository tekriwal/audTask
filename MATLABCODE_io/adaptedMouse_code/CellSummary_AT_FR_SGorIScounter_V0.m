%AT 9/29 - attempting to make a version of CellSummary in which I can link
%FR's to whether trial type was SG or IS
...
function [fig_handle, raster_info] = CellSummary_AT_FR_SGorIScounter(behav_file, spk_file, align_ind, window,...
    num_trials_back, trials_to_include)
...
GetPhysioGlobals; % accesses global variables associated with physiology/behavior data
...
%AT 1/8/19 - need to change the loading part of this fx since I'm now
%passing a variable rather than a path for the inputs (working with
%AT_CellActivity_folder_m_V2)
...
%AT 2/14/19 - changing back to original parameters below because of
%taskvalve adapter use up top
...
for i = 1:num_stim
    ...
    if num_trials > 0 % make sure there are trials of this type
        ...
        %%AT commenting out the below title portion on 2/14/19
%         % On the first time through this loop, add the plot title
%         if i == 1
%             title(spk_file); %AT 2/14/19 - oh man, I think that there is an error being formed by including the use of a title which has backslashes
%         end
...
%     
%     %AT 12/11/18 adding below for one of bp15s days (note, didnt end up
%     %helping.
%     if isnan(raster_h)
%         raster_u = 0;
%     end

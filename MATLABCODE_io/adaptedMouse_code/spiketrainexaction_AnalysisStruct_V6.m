%6/11/20, V6 has side of surgery as input so that we can determine
%ipsi/contra

%6/3/20 V5; adding in output for correct vs incorrects, previous version of
%code worked well for what I wanted it to do.
...
if strcmp('L', surgerySide)
    spiketrainStrct_V2.SG.ipsi = spikestruct;    
elseif ~strcmp('L', surgerySide)
    spiketrainStrct_V2.SG.contra = spikestruct;    
end
% %
...
trial_inds = find(taskbase_io.actual == taskbase_io.response);
trial_start_times = taskbase_io.events.trialStart(trial_inds)';
% trial_start_times_AO = taskbase_io.trialStart_AO(trial_inds)';
trialStart_times = (taskbase_io.trialStart_AO(trial_inds)')/(Ephys_struct.mer.sampFreqHz);
upPressed_times = taskbase_io.events.upPressed(trial_inds)';
...
spiketrainStrct_V2.corrects = spikestruct;
...
trial_inds1 = find((taskbase_io.response == 1) & (taskbase_io.actual == 2));
trial_inds2 = find((taskbase_io.response == 2) & (taskbase_io.actual == 1));
trial_inds = sort([trial_inds1;trial_inds2]);
...
spiketrainStrct_V2.incorrects = spikestruct;

%%
% 
% %AT 6/20/20, I think that I carry out this exact calc already up above,
% %going to comment it out - hopefully nothing weird happens.
% %AT adding below on 6/11/20
% trial_inds = find((taskbase_io.response == 2) & (nochoice_trials == 1));
% trial_start_times = taskbase_io.events.trialStart(trial_inds)';
% % trial_start_times_AO = taskbase_io.trialStart_AO(trial_inds)';
% trialStart_times = (taskbase_io.trialStart_AO(trial_inds)')/(Ephys_struct.mer.sampFreqHz);
...
% % end
%
% % % %AT 4/24/20, for burst analysis we want the whole trial
% % % epochs = {[1 7]};
%
%
...
%AT adding below to save out the taskbase.io struct for use in logistic
%regression 7/26/20

filename22 = (['/taskbase_io_', caseName, spikeFile, clustname]);

if saveFig == 1
    save(fullfile(figuresdir, filename22), 'taskbase_io');
end

...
%AT V2 on 3/28/20; goal is to try and uniform the size of the rasters
...
function [ref_spike_times, trial_inds, plot_handles, ref_spike_times_bursting] =...
    raster_io_burstAnalysis_V2(NUM_TRIALS_TO_PLOT, spike_times, trial_start_times, event_times, window, secondary_event_times, ref_time_start_offset, ref_time_end_offset)
...
for trial_num = 1:length(trial_start_times)
    ...
    ref_time_start = trial_start_times(trial_num) + event_times(trial_num) + (ref_time_start_offset); % absolute time of event to reference spikes to
    ref_time_end = secondary_event_times(7, trial_num) + (ref_time_end_offset); % absolute time of event to reference spikes to
    ...
    if ~isnan(ref_time_start) % ref_time will be NaN if the particular event did not occur in this trial
        ...
        trial_spike_times = spike_times(find((spike_times >= (ref_time_start)) &...
            (spike_times <= (ref_time_start + ref_time_end)))) - ref_time_start;
        
        ref_spike_times = [ref_spike_times; trial_spike_times(:)];
        ref_spike_times_bursting{trial_num} = trial_spike_times(:);

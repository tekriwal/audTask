%editted for case 13 to exclude some trials in which pt zoned out, refer to
%excel sheet notes
%AT creating V2 on 6/24/20 to further explore behavior. Old files will
%remain the same, updating new output as 'V2's. Ephys code will continue to
%pull from V1's due to some accomodations that were made.
...
%6/28/20 differentiating w/ helper fx between OE and nearOE so should be
%fine
...
[taskbase_io, ~, index_errors, index_errorsPtHeard] = behav_file_adapter_V2(rData,inputmatrix, cutfirst3trials, cutNearOEs);


% AT 6/24/20, adding below
errorsPtHeard = sum(index_errors)-sum(index_errorsPtHeard);
...
[choice_trials_L, choice_trials_R, nochoice_trials_L, nochoice_trials_R, choice_trials_L_corrects, choice_trials_R_corrects, nochoice_trials_L_corrects, nochoice_trials_R_corrects, errors ] = io_taskindexing_AT_V4(rData, inputmatrix, exclusionFilter, index_errorsPtHeard);
...
behavior.(caseName).mean_rxnTime.SG = mean(submitsResponse_times - goCue_times);

behavior.(caseName).rxnTime_wholetrial.SG = feedback_times - upPressed_times;
behavior.(caseName).mean_rxnTime_wholetrial.SG = mean(feedback_times - upPressed_times);

behavior.(caseName).rxnTime_leftUPtillFeedback.SG = feedback_times - leftUP_times;
behavior.(caseName).mean_rxnTime_leftUPtillFeedback.SG = mean(feedback_times - leftUP_times);
...
behavior.(caseName).errors_errorsPtHeard.SG = errors.SGerrors_errorsPtHeard;

behavior.(caseName).percerrors.SG = errors.SGerrors/(length(trial_inds)+errors.SGerrors);
behavior.(caseName).percerrors_errorsPtHeard.SG = errors.SGerrors_errorsPtHeard/(length(trial_inds)+errors.SGerrors);
...
if strcmp(caseName, 'case13')
    behavior.(caseName).rxnTime.IS(33) = [];
    behavior.(caseName).rxnTime.IS(28) = [];
    behavior.(caseName).rxnTime.IS(15) = [];
end

behavior.(caseName).mean_rxnTime.IS = mean(behavior.(caseName).rxnTime.IS);

behavior.(caseName).rxnTime_wholetrial.IS = feedback_times - upPressed_times;
if strcmp(caseName, 'case13')
    behavior.(caseName).rxnTime_wholetrial.IS(33) = [];
    behavior.(caseName).rxnTime_wholetrial.IS(28) = [];
    behavior.(caseName).rxnTime_wholetrial.IS(15) = [];
end

behavior.(caseName).mean_rxnTime_wholetrial.IS = mean(behavior.(caseName).rxnTime_wholetrial.IS);



behavior.(caseName).rxnTime_leftUPtillFeedback.IS = feedback_times - leftUP_times;
if strcmp(caseName, 'case13')
    behavior.(caseName).rxnTime_leftUPtillFeedback.IS(33) = [];
    behavior.(caseName).rxnTime_leftUPtillFeedback.IS(28) = [];
    behavior.(caseName).rxnTime_leftUPtillFeedback.IS(15) = [];
end

behavior.(caseName).mean_rxnTime_leftUPtillFeedback.IS = mean(behavior.(caseName).rxnTime_leftUPtillFeedback.IS);
...
behavior.(caseName).errors_errorsPtHeard.IS = errors.ISerrors_errorsPtHeard;

behavior.(caseName).percerrors.IS = errors.ISerrors/(length(trial_inds)+errors.ISerrors);
behavior.(caseName).percerrors_errorsPtHeard.IS = errors.ISerrors_errorsPtHeard/(length(trial_inds)+errors.ISerrors);


if exclusionFilter == 0 && cutfirst3trials == 1
    save('/Users/andytek/Box/Auditory_task_SNr/Data/generated_analyses/behavior_IO/behavior_struct_IO_nofilter_V2.mat', 'behavior')
elseif exclusionFilter == 1 && cutfirst3trials == 1
    save('/Users/andytek/Box/Auditory_task_SNr/Data/generated_analyses/behavior_IO/behavior_struct_IO_no5050s_V2.mat', 'behavior')
elseif exclusionFilter == 2 && cutfirst3trials == 1
    save('/Users/andytek/Box/Auditory_task_SNr/Data/generated_analyses/behavior_IO/behavior_struct_IO_no5050snoHardSG_V2.mat', 'behavior')
elseif exclusionFilter == 0 && cutfirst3trials == 0
    save('/Users/andytek/Box/Auditory_task_SNr/Data/generated_analyses/behavior_IO/behavior_struct_IO_nofilter_keepalltrials_V2.mat', 'behavior')
elseif exclusionFilter == 1 && cutfirst3trials == 0
    save('/Users/andytek/Box/Auditory_task_SNr/Data/generated_analyses/behavior_IO/behavior_struct_IO_no5050s_keepalltrials_V2.mat', 'behavior')
elseif exclusionFilter == 2 && cutfirst3trials == 0
    save('/Users/andytek/Box/Auditory_task_SNr/Data/generated_analyses/behavior_IO/behavior_struct_IO_no5050snoHardSG_keepalltrials_V2.mat', 'behavior')
end

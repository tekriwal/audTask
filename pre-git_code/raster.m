...
SPIKE_TICK_WIDTH = 0.1; % width of lines marking spikes
EVENT_TICK_WIDTH = 3; % width of lines marking secondary events (e.g., odor poke out)
...
if no_plot_flag == 0 % do the plotting
    ...
    %     UNCOMMENT BELOW TO PUT IN ZERO MARKERS
    
    hold on
    where_toput_line = -[1,length(trial_start_times)];
    x = [0,0];
    %     y=[where_toput_line,where_toput_line];
    y = where_toput_line;
    line(x,y,'LineWidth',3, 'Color', 'b')
    
    %     UNCOMMENT BELOW TO PUT IN LINES FOR DIFF BLOCKS
    %     hold on
    %     where_toput_line = -9.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'r')
    %     hold on
    %
    %     where_toput_line = -18.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'b')
    %     hold on
    %
    %     where_toput_line = -27.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'r')
    %     hold on
    %
    %     where_toput_line = -36.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'b')
    %     hold on
    %
    %     where_toput_line = -45.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'r')
    %     hold on
    %
    %     where_toput_line = -54.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'b')
    %     hold on
    %
    %     where_toput_line = -63.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'r')
    %     hold on
    %
    %     where_toput_line = -70.5;
    %     x = [-0.2, 0.2];
    %     y=[where_toput_line,where_toput_line];
    %     line(x,y,'LineWidth',1, 'Color', 'b')
    %
    ...
    if size(secondary_event_times, 1) > 0
        for secondary_event_ind = 1:size(ref_secondary_event_times, 2)
            ...
            plot_handle_sec_events(:, secondary_event_ind) =...
                ...
                'Color', 'r', 'LineWidth', EVENT_TICK_WIDTH);

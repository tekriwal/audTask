...
feedback_index = zeros(2,length(cutTTL));
...
for i = stopgap:(length(cutTTL)-1)
    if cutTTL(i+1) - cutTTL(i) < 400 && cutTTL(i) - cutTTL(i-10) < 28000
        feedback_index(1,i) = 1;
        feedback_index(2,i) = cutTTL(i+4); %THE PLUS FOUR IS BECAUSE 4th TTL after that last one of 11 is played right after feedback is given
    end
end

% little aside here to make the waitperiod_index more scannable by eye
feedback_index_easilyscannable = [];
for i = 1:length(feedback_index)
    if feedback_index(1,i) == 1
        appendme = feedback_index(2,i);
        feedback_index_easilyscannable = [feedback_index_easilyscannable appendme];
    end
...
if sum(feedback_index(1,:)) ~= (sum(redundancy_index(1,:))+sum(redundancy_index(2,:))) 
    disp('ERROR WITH CALC1');
elseif sum(feedback_index(1,:)) == (sum(redundancy_index(1,:))+sum(redundancy_index(2,:)))
...
behavioral_matrix = zeros((length(rData)),6);
...
condition = behavioral_matrix(:,5)==3; %this says, if column five has a 3 in it
behavioral_matrix(condition,:) = []; %remove the whole row

referencetimes_column = feedback_index_easilyscannable';
...
% ylim([0 1])
% yticks([0 0.5 1])
ylabel('Number spikes (ave. moving window)');
% xticks([-1000 -800 -600 -400 -200 0 200 400 600 800 1000])
xlabel('Time (each bin = 10 milliseconds)')

%PART 1 - ephys analysis; use on the older versions of the neural data
...
% first TTL after the wait period is completed
...
load('rData_PT3.mat')
load('AbvTrgt_40_00910.mat')
load('spkData_a40_00910_3_clzD.mat')
load('inputshorten_PT3.mat')
rData = rData_PT3;

input_matrix_rndm = inputshorten_PT3;
...
for i = stopgap:(length(TTLdata)-1)
    if TTLdata(i+1) - TTLdata(i) < 400 && TTLdata(i) - TTLdata(i-10) < 27000
...
smoothingfactor = 40;
bin_resolution = 4; %MILLISECONDS length for a given bin. This will determine how frequency is plotted!!!!!
...
timepointwanted = 10; %so this corresponds as follows: 9 is for the 'start loop' aka when up was pressed, 10 is the go beep, 11 is when response given
